...
|`absolute` | Optional. Use this field to determine whether the metric name is the exact name specified in the `name` field or with the package prefix.
|===
...
|===
| `unit` | Set the unit of the metric, if it is NONE, the metric name is used as is without appending the unit name and no scaling is applied.
|===
