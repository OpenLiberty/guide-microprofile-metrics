...
the `MetricRegistry` directly. You will learn how to use annotations to register metrics to the
...
the application metrics in Prometheus format. To see only the application metrics, first point your browser to
`http://localhost:9080/inventory/systems` and then to `https://localhost:9443/metrics/application`
...
* The default value of the `monotonic` field is `false` which means that the counter is incremented
 while the annotated method is invoked and decremented after the annotated method returns. However, set the value to `true` to only increment the counter.

 Note, the `@Counted` and `@Timed` annotations can be also placed in the Resource as
 well, such as `InventoryResource`.
...
Note, when the value of the gauge is retrieved, the underlying annotated method `getTotal()` is
...
`https://localhost:9443/metrics/application` endpoint, which is provided for you.
