...
You will include metrics in an application to pinpoint issues, provide long-term trend data for
...
Because these metrics are application-related, you have to first to point to the `http://localhost:9080/inventory/systems`
URL, otherwise they will not appear. See sample outputs for the `@Timed`, `@Gauge`, and `@Counted` annotations:
...
the `org.eclipse.microprofile.metric` group ID. This feature allows you to use the MicroProfile Metrics API to provide
metrics from your microservices. Also, in the `pom.xml` file, you will find the `javax.net.ssl.trustStore` system
property variable. This variable points to the SSL certificate of the system.
...
| `monotonic` | if `true`, the counter counts the number of invocations of the annotated method. If `false`, the counter counts the
number of currently executing invocations of the annotated method. In other words, it makes the counter increment
when the annotated method is called and decrement when it returns.
|===
...
This object is queried for metadata and metrics when you call the `https://localhost:9443/metrics/application`
MicroProfile Metrics endpoint, which is provided for you. Also, the `@Counted`, `@Gauge`, and `@Timed` annotations can be
used in services, such as the `inventory` service in the
...
See the breakdown of the test cases:
