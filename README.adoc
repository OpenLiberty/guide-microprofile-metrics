...
* The default value of the `monotonic` field is `false`, which means that the counter is incremented
 while the annotated method is invoked and decremented after the annotated method returns. However, set the value to `true` to increment only the counter.

The `@Counted` and `@Timed` annotations can be also placed in the Resource as
...
When the value of the gauge is retrieved, the underlying annotated method `getTotal()` is
