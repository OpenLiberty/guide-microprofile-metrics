...
|`absolute` | Optional. If true, set the metric name to the exact name that is specified in the `name` field. If false, set the metric name to the given `name` with the prefix of the package class name.
|===
...
|===
| `unit` | Use this field to scale the metric, if it is NONE, the metric name is used as is without appending the unit name and no scaling is applied. Otherwise, the metric value should be scaled to the base unit of the respective known unit family, for example, unit family `MetricUnits.Percent` with base unit `ratio`.
|===
