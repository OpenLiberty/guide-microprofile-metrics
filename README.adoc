...
The MicroProfile Metrics API was added as a dependency to your [hotspot=1-240 file=0]`pom.xml` file. This is the dependency with
the [hotspot=91 file=0]`mpMetrics` artifact ID. This dependency was added to use the MicroProfile Metrics API
in your code to provide metrics from your microservices.

[role="code_command hotspot=1-18 file=1", subs="quotes"]
----
Create the server configuration file.
`src/main/liberty/config/server.xml`
----

The [hotspot=8 file=1]`mpMetrics` feature enables MicroProfile Metrics support in Open Liberty. Note that this
feature requires SSL and the configuration has been provided for you.

The [hotspot=13 file=1]`quickStartSecurity` and [hotspot=14 file=1]`keyStore` configuration elements provide basic security to secure the
...
[role="code_command hotspot=33-67", subs="quotes"]
----
...
Apply the [hotspot=38-41]`@Timed` annotation to the [hotspot=40-42]`get()` method to track how frequently the method is
invoked and also how long it takes for the invocation to complete. This annotation has these
metadata fields:

|===
|[hotspot=38]`name` | Optional. Use this field to name of the metric.
|[hotspot=39]`absolute` | Optional. Use this field to determine whether the metric name is the exact name that is specified in the `name` field or that is specified with the package prefix.
|[hotspot=40-41]`description` | Optional. Use this field to describe the purpose of the metric.
|===

Apply the [hotspot=57-60]`@Counted` annotation to the [hotspot=61-63]`list()` method to count how many times the
`http://localhost:9080/inventory/systems` URL is accessed. Note the additional metadata field:

|===
| [hotspot=59]`monotonic` | Set this annotation to `true` to count the total number of invocations of the
annotated method, which is the total number of times the inventory is accessed.
|===

Apply the [hotspot=65-68]`@Gauge` annotation to the [hotspot=69-71]`getTotal()` method to track the number of systems that are stored in
the inventory. Note that when the value of the gauge is retrieved, the underlying [hotspot=69-71]`getTotal()` method
is called to return the size of the inventory. Note the additional metadata field:

|===
| [hotspot=65]`unit` | Set the unit of the metric. If it is [hotspot=65]`MetricUnits.NONE`, the metric name is used as is without appending the unit name, and no scaling is applied.
|===
...
[role="code_command hotspot=29-144", subs="quotes"]
----
...
* The [hotspot=68-78 file=0]`testPropertiesRequestTimeMetric()` test case validates the [hotspot=38-41 file=1]`@Timed` metric. It sends a request to the
...
* The [hotspot=80-89 file=0]`testInventoryAccessCountMetric()` test case validates the [hotspot=57-60 file=1]`@Counted` metric. The test case sends a request to the
...
* The [hotspot=91-100 file=0]`testInventorySizeGaugeMetric()` test case validates the [hotspot= file=1]`@Gauge` metric. The test case first ensures
that the localhost is in the inventory. It then looks for the [hotspot= file=1]`@Gauge` metric and asserts
that the inventory size is equal to 1.

The [hotspot=43-48 file=0]`oneTimeSetup()` method retrieves the port number for the server and builds a base URL string
to set up the tests. Apply the [hotspot=42 file=0]`@BeforeClass` annotation to this method to execute it before any of
the test cases.

The [hotspot=51-54]`setup()` method creates a JAX-RS client that makes HTTP requests to the `inventory` service.
Register this client with a [hotspot=53 file=0]`JsrJsonpProvider` JSON-P provider to process JSON resources. The
[hotspot=57-59]`teardown()` method destroys this client instance. Apply the [hotspot=50 file=0]`@Before` annotation so that a method
executes before a test case, and apply the [hotspot=56 file=0]`@After` annotation so that a method executes after a test
case. Apply these annotations to methods that are generally used to perform any setup and teardown tasks
before and after a test.

To execute the test cases in a particular order, put them in a [hotspot=62-66 file=0]`testSuite()` method.
Label the method with the [hotspot=61]`@Test` annotation, which automatically executes when your test class runs.
...
To see whether the tests detect a failure, go to the [hotspot=29-144 file=0]`MetricsTest.java` file and change any of the assertions
