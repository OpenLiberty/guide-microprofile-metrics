...
The MicroProfile Metrics is a feature that provides an endpoint where you can get the metrics
...
After you complete this guide, you will create the metrics endpoint:
...
* `@Timed` annotation on the `get()` method which tracks how frequently the method is
invoked and also how long it took for the invocations to complete. Most of the annotations are
...
* `@Counted` annotation on the `list()` method which counts how many times the list of hosts is
requested. In other words, how many times we display the current contents of the inventory by
going to `/inventory/hosts` endpoint. Like the `@timed` annotation, this annotation
has a `name` and a `description` field too. The `monotonic` field has a default value `false` which
means that the counter is incremented before the annotated method returns, counting current
invocations of the annotated method. This is not what we want in this case, thus set the value
to `true`.

* `@Gauge` annotation on the `getHostCount()` method which tracks the number of hosts stored in the
inventory by visiting `/inventory/hosts/localhost` endpoint. A value should be
provided for the `unit` because there is no default value for this annotation. The `name` and
`description` are optional like in the previous two annotations.
