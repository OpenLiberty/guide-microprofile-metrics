...
Apply the [hotspot=42-47]`@Counted` annotation to the [hotspot=45-47]`list()` method to count how many times the
http://localhost:9080/inventory/systems[http://localhost:9080/inventory/systems^] URL is accessed. 

Apply the [hotspot=49-52]`@Gauge` annotation to the [hotspot=54-56]`getTotal()` method to track the number of systems that are stored in
