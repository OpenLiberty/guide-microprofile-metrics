...
:page-tags: ['Metrics' , 'MicroProfile' , 'CDI', '@Timed', '@Counted', '@Gauge', 'microservices']
...
Next, point your browser to the `https://localhost:9443/metrics` MicroProfile Metrics endpoint. If you
are faced with a security exception, bypass it by selecting `Advanced` or `Details` and clicking on
the link to proceed to the webpage. Log in as the `admin` user with `adminpwd` as the password. 
