...
hosts are not currently registered. Point to `http://localhost:9080/inventory/systems/localhost` URL to add a host. Next, Point to the
...
`@Counted`, `@Guage` `@Timed` annotations can be placed in the Resource, such as
`InventoryResource`.
