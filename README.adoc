...
You will use MicroProfile Metrics have endpoints where you can get the metrics
...
the standard metrics and the application metrics in a text format.
 ...
 These are some of the metric outputs: 
...
Because these metrics are application-related, you had
...
use the MicroProfile Metrics API to provide metrics from your microservices. Also, the 
`javax.net.ssl.trustStore` system property is set to point to the SSL certificate.
...
Notice these metrics annotations:
...
you request the list of systems methods when you visit the `http://localhost:9080/inventory/systems`
URL that retrieves the information for a list of all previously registered hosts. These are the 
...
This is the additional metadata field: 
...
Point your browser to the `https://localhost:9443/metrics` URL to find the metrics that shows JVM 
and server base details. 

Next, point your browser to `http://localhost:9080/inventory/systems`. After that, you can find 
...
These are the break down of the test cases:

* Use the `testGetPropertiesTime()` test case to test the `@Timed` annotation. This test case
...
* Use the `testListCount()` test case to test the `@Counted` annotation. This test case uses the 
...
* Use the `testInventorySize()` test case to test the `@Gauge` annotation. This test case uses the 
`connectToEndpoint()` method to send a request to the `http://localhost:9080/inventory/systems/localhost`
URL. The inventory size increases because of the request. The `getHostCount()` method in the 
`InventoryManager` class returns one host, which represents the localhost. The request then asserts 
the host with the `validateMetric()` method.
