...
failure whenever a code change introduces a defect. JUnit and the restfulWS Client API provide a simple
...
The [hotspot=setup file=0]`setup()` method creates a restfulWS client that makes HTTP requests to the `inventory` service. The
