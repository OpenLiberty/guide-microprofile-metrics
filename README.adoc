...
The [hotspot=quickStartSecurity file=1]`quickStartSecurity` configuration element provides basic security to secure the Liberty instance. When you visit the `/metrics` endpoint, use the credentials defined in the Liberty's configuration to log in and view the data.
...
// cloud-hosted guide instructions:
ifdef::cloud-hosted[]
Run the following curl command to review all the metrics that are enabled through MicroProfile Metrics. You see only the system and vendor metrics because the instance just started, and the ***inventory*** service has not been accessed.
```bash
curl -k --user admin:adminpwd https://localhost:9443/metrics
```
...
The [hotspot=oneTimeSetup file=0]`oneTimeSetup()` method retrieves the port number for the Liberty runtime and builds a base URL string to set up the tests. Apply the [hotspot=BeforeAll file=0]`@BeforeAll` annotation to this method to run it before any of the test cases.
