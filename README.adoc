...
image::resources/outputExample.png[]

Because these metrics are application-related, you had
to point first to `http://localhost:9080/inventory/systems` URL otherwise they will not appear.
...
* The `absolute` field gives this annotation the name of the method.
* The `description` field is optional and describes the purpose of the metric annotation.
* The `monotonic` field is set to `true` so that the counter will only increment.
...
When the value of the gauge is retrieved, the underlying `getTotal()` annotated method is
